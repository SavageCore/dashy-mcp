...
_OP_GROUP = {op: group for group, ops in dashy_mcp._GROUPS.items() for op in ops}
...
def test_all_operations_grouped():
    ...
    grouped_names = [n for names in dashy_mcp._GROUPS.values() for n in names]
    assert len(grouped_names) == len(set(grouped_names))
    for n in grouped_names:
        assert hasattr(dashy_mcp, n), f"{n} not found in dashy_mcp module"


async def test_group_tools_are_the_only_registered_tools(server):
    async with Client(server) as c:
        tools = await c.list_tools()
    assert {t.name for t in tools} == set(dashy_mcp._GROUPS)
...
async def test_unknown_operation_rejected_by_schema(server):
    ...
    with pytest.raises(ToolError, match="validation error"):
        async with Client(server) as c:
            await c.call_tool("dashy_key", {"operation": "not_a_real_operation"})
